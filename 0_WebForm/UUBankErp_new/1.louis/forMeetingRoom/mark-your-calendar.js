(function ($) {
    ...
    //if (date == "2021-11-30" && time == "上午") {
    //    $(".myc-available-time").attr("hidden", "hidden");
    //}
        ...
        this.getAvailableTimes = function () {
            ...
            for (i = 0; i < 7; i++) {
                ...
                $.each(settings.availability[i], function () {
                    tmpAvailTimes += `
                        <a href="javascript:;" class="myc-available-time" id="`+ formatDate(settings.startDate.addDays(i)) +this+ `" data-time="` + this + `" data-date="` + formatDate(settings.startDate.addDays(i)) + `">
                            ` + this + `</a>`;
                });
                tmp += `
