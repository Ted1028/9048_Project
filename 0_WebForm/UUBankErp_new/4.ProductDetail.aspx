...
<script runat="server">
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            CartId=1,
            Id = int.Parse(Lb_BY_Id.Text),
            Waterid = SP.Waterid,
            Count = int.Parse(PDcount.Value)
        };

        CartUtility.AddCart(cart);

        ProductUtility.AddCountNow(SP.Waterid, int.Parse(PDcount.Value));
        Response.Redirect($"~/4.Product_Home_AJAX.aspx?id={int.Parse(Lb_BY_Id.Text)}");
        //Response.Redirect($"~/4.Product_Home_AJAX.aspx?id=6");
